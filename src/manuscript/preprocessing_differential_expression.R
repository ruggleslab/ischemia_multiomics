################################################################################
# Differential Expression Analysis - RNA-seq Data Processing
#
# Description: Process RNA-seq count data, perform QC filtering, normalization,
#              differential expression analysis using DESeq2, and gene set enrichment
#
# Analyses:
#   - Inducible ischemia severity: DEGRISCH, IMGDEGIS (0-1=none-mild, 2=moderate, 3=severe)
#   - CAD severity: CTMULT50, CTANYD50, DUKESCORE groups
################################################################################

## Load Libraries
packagelist <- c("ggplot2", "reshape2", "DESeq2", "grid", "gridExtra", "scales", "ggrepel", "tools")
suppressMessages(lapply(packagelist, function(xxx)
...
# TODO: Move metafile path to config.R
...
## Find the overlapping SAMPLES between the metatable and countdata
...
## Remove bad samples from previous run
# TODO: Move badsampfile path to config.R
...
## Remove hemoglobin genes from analysis
counttabfilt2 <- counttabfilt2[!rownames(counttabfilt2) %in% rmgenelist,]
dim(counttabfilt2)


##### Filter for Gene Count
# Genes must have at least 16 counts in at least half of samples
mincountcutoff <- 16
...
## Change non-character columns to characters for plotting
...
## Write out the filtered count table and metadata
...
##### Plot the top genes that are proportionately representative
numgenesplot <- 50
...
## Create a table of top varied genes for plotting
# Select genes between 95th-99.99th percentile of variance to exclude extreme drivers
...
## Create a table of top varied genes post-normalization
...
## Iteratively sort by each metadata entry and display
...
## Flag samples that are outliers by correlation (>3 SD from mean)
...
## Summarize results by counting how many tests the sample failed
...
## With ASR matching - exclude SEX comparisons
...
for (deseqanalysisnum in seq_len(length(DEseqreslist))) {
    ...
    for (runnum in seq_len(length(summaryparams))) {
        ...
        if (nrow(GOIhmtab) > 0) {
            ...
            ## Add the metadata back in to check for confounding variables
...
## Batch exploration
# TODO: Move batchexplorationoutfilepath to config.R
...
# TODO: Move biorepintablefile path to config.R
